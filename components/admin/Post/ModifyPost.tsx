...
import { Calendar, Save,ChevronDown,Trash2,Plus,ExternalLink,Link,Pencil,
         ...
         Code2,FileBraces,Image,LinkIcon,CheckCircle2,CircleAlert,ShieldXIcon,FileText,Download
        } from "lucide-react";
...
export default function ModifyPost({Post, onNavigate }: {Post?: BasePost, onNavigate?: (tab: string) => void;}){
  ...
  const addTextAreaBlock = async () => {
    setContent([...content, { type: "textAreaBlock", content: "" }]);
  };

  const addVideo = async (contents: Gallery[])=>{
    setContent([...content, { type: "youtube", content: contents}]);
  }

  const addgithub = async ()=>{
    setContent([...content, { type: "github", content: {link: "",description: "",text: ""} as GitHubInfo}]);
  }

  const addImages = async (Item: Gallery[]) =>{
    setContent([...content,{type:"image",content: Item}])
  }

  const addFile = async (Item: DriveFile) =>{
    setContent([...content,{type:"file",content: Item}])
  }

  const addLink = async (Item: Item[]) =>{
    setContent([...content,{type:"link",content: Item}])
  }
  ...
  const addCode = async () =>{
    setContent([...content,{type:"code",content: {fileName: "",code: ""} as CodeInfo}])
  }
        ...
        <div onClick={()=>{if(showLanguage){setShowLanguage(false);}}} className="max-w-4xl mx-auto py-10 px-6 bg-zinc-950 text-zinc-100 min-h-screen">
            ...
            <header className="space-y-6 mb-2">
              ...
              <textarea
                ...
                onChange={(e)=>setPost(prev => prev ? ({ ...prev, description: e.target.value }) : prev)}
          ...
          <div className="mt-3">
            ...
            <div className="space-y-2 mt-2">
              {content.map((block, index) => (
                <div key={index} className="group relative p-6 bg-zinc-900/50 border border-zinc-800 rounded-2xl hover:border-zinc-700 transition-all">
                  ...
                  <button onClick={()=>setContent((prev)=>prev.filter((_, i) => i !== index))} className="cursor-pointer absolute -right-3 -top-3 p-2 bg-red-900/20 text-red-500 rounded-full opacity-0 group-hover:opacity-100 transition hover:bg-red-900/40">
                    <Trash2 size={14} />
                  </button>
                ...
                return (
                  <div className="group relative flex items-center justify-between p-5 bg-zinc-900/40 backdrop-blur-md border border-zinc-800 rounded-2xl hover:border-blue-500/30 transition-all duration-300 mb-4">
                    ...
                    <a 
                      ...
                      rel="noopener noreferrer"
                      download
                      className="flex cursor-pointer items-center gap-2 px-5 py-2.5 bg-white hover:bg-zinc-200 text-black text-sm font-bold rounded-xl transition-all active:scale-95 shadow-lg shadow-white/5"
                    >
                      <Download size={18} />
            ...
            {ChangeImage &&(
              <ChangeImageComponent indexBlock={ChangeImage.indexBLock} indexImage={ChangeImage.indexImage} onClose={() => setChangeImage(null)} setBlocks={setContent} blocks={content} />
            )}
