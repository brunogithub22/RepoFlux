...
export default function LoadList({ onClose, onSave }: { onClose: () => void, onSave: (link: Item[]) => void }){
            ...
            <div>
                <div className="flex-1 overflow-y-auto p-6 space-y-3 custom-scrollbar">
                  {listItem.length > 0 ? (
                     listItem.map((item, idx) => (
                      <div key={idx} className="flex items-center gap-4 p-4 rounded-2xl bg-zinc-800/30 border border-zinc-800 hover:border-zinc-700 transition-all">
                        ...
                        <button 
                           ...
                           onClick={() => setlistItem(listItem.filter((_, i) => i !== idx))}
                        >
                            <Trash2 size={18} />
                        </button>
                     </div>
                     ...
                     <div className="text-center py-10 text-zinc-600">No item added yet.</div>
                   )}
                </div>
                ...
                {showFormList && (
                    <ShowItem isLink={false} onClose={() => setShowFormList(false)} onSave={getData}  />
                )}
            </div>
        );
}